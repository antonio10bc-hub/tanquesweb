...
function create() {
    ...
    if (window.initialGameData) {
        ...
        if (window.initialGameData.players) {
            Object.keys(window.initialGameData.players).forEach((id) => {
                if (window.initialGameData.players[id].playerId === socket.id) addPlayer(self, window.initialGameData.players[id]);
                else addOtherPlayer(self, window.initialGameData.players[id]);
            });
        }
        if (window.initialGameData.walls) window.initialGameData.walls.forEach(p => makeWallVisible(self, p.x, p.y, false));
    }

    socket.on('wallGroupRevealed', (blockLists) => blockLists.forEach(p => makeWallVisible(self, p.x, p.y, true)));
    socket.on('newPlayer', (info) => addOtherPlayer(self, info));
    socket.on('disconnectPlayer', (id) => { if (self.enemies[id]) { self.enemies[id].destroy(); delete self.enemies[id]; } });
    ...
    socket.on('gameReset', (data) => {
        ...
        if (!self.player) addPlayer(self, myInfo);
            ...
            self.player.setVisible(true); self.player.setPosition(myInfo.x, myInfo.y);
            self.player.rotation = myInfo.rotation; setupColliders(self); updateTankAnimation(self.player);
        }
        Object.keys(data.players).forEach(id => {
            if (id !== socket.id) {
                if (self.enemies[id]) {
                    self.enemies[id].setVisible(true); self.enemies[id].setPosition(data.players[id].x, data.players[id].y);
                    self.enemies[id].rotation = data.players[id].rotation; updateTankAnimation(self.enemies[id]);
                } else addOtherPlayer(self, data.players[id]);
            }
        });
    });
...
function update(time, delta) {
    ...
    if (this.shootBtn.isDown) {
        if (!this.shootBtn.locked) { isShooting = true; this.shootBtn.locked = true; }
    } else this.shootBtn.locked = false;
...
function createTankTexture(scene, name, colorMain, colorShadow) {
    ...
    g.fillStyle(colorShadow, 1); g.fillRoundedRect(0, sh, w, h, 6);
    g.fillStyle(colorMain, 1); g.fillRoundedRect(0, 0, w, h, 6);
    g.fillStyle(0xffffff, 0.3); g.fillRoundedRect(w - 12, h/2 - 3, 12, 6, 2);
    g.generateTexture(name, w, h + sh);
}
function createWallTexture(scene, name, colorMain, colorShadow) {
    const s = 40, sh = 6;
    const g = scene.make.graphics({ x: 0, y: 0, add: false });
    g.fillStyle(colorShadow, 1); g.fillRect(0, 0, s, s);
    g.fillStyle(colorMain, 1); g.fillRect(0, 0, s, s - sh);
...
function setupTankAnimations(scene, sprite) {
    sprite.idleTween = scene.tweens.add({ targets: sprite, scaleX: 1.05, scaleY: 1.05, duration: 500, yoyo: true, repeat: -1, ease: 'Sine.easeInOut', paused: true });
    sprite.moveTween = scene.tweens.add({ targets: sprite, scaleX: 1.02, scaleY: 1.02, duration: 200, yoyo: true, repeat: -1, ease: 'Sine.easeInOut', paused: true });
...
function addPlayer(self, info) {
    ...
    self.player.setCollideWorldBounds(true); self.player.rotation = info.rotation;
    self.player.body.setSize(36, 26); setupColliders(self); setupTankAnimations(self, self.player);
}
function addOtherPlayer(self, info) {
    const other = self.physics.add.sprite(info.x, info.y, 'enemyTex');
    other.playerId = info.playerId; other.rotation = info.rotation; self.enemies[info.playerId] = other;
    other.body.setSize(36, 26); setupTankAnimations(self, other);
}
function fireBullet(scene, source, isMyBullet) {
    const vec = new Phaser.Math.Vector2(); vec.setToPolar(source.rotation, 25);
    ...
    scene.bullets.add(bullet); scene.physics.velocityFromRotation(source.rotation, 600, bullet.body.velocity);
    if (isMyBullet) Object.values(scene.enemies).forEach(enemy => scene.physics.add.overlap(bullet, enemy, (b, e) => { b.destroy(); socket.emit('playerDied', e.playerId); }));
    scene.physics.add.overlap(bullet, scene.walls, (b, w) => { b.destroy(); if (!w.visible) socket.emit('wallHit', { x: w.coordX, y: w.coordY }); });
}
function setupColliders(self) {
    if (self.player && self.walls) self.physics.add.collider(self.player, self.walls, (player, wall) => { if (!wall.visible) socket.emit('wallHit', { x: wall.coordX, y: wall.coordY }); });
}
function makeWallVisible(self, x, y, animate) {
    self.walls.getChildren().forEach(wall => {
        if (wall.coordX === x && wall.coordY === y && !wall.visible) {
            ...
            if (animate) { wall.alpha = 0; self.tweens.add({ targets: wall, alpha: 1, duration: 300 }); }
        }
    });
}
function showGameOverUI(scene, playerWon) {
    ...
    let title = scene.add.text(400, 250, playerWon ? "¡VICTORIA!" : "DERROTA", { fontSize: '60px', fontFamily: 'Arial Black', color: color, stroke: '#000000', strokeThickness: 6 }).setOrigin(0.5).setScale(0);
    ...
    btn.on('pointerdown', () => { btnText.setText("Reiniciando..."); socket.emit('requestRestart'); });
